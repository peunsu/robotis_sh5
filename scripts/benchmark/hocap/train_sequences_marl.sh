#!/usr/bin/env bash
# =============================================================================
# train_sequences_marl.sh — MAPPO pipeline for HO-Cap: dataset → IK → pretrain
# → train.
#
# Multi-agent variant of train_sequences.sh. For each HO-Cap sequence:
#   1. Checks the processed mano data is present.
...
#   data/processed/hocap/ffw_sh5_marl/right/<trajectory_task>/<data_id>/
...
for key in "${SEQUENCES[@]}"; do
    ...
    if [[ ! -d "${MANO_DIR}" ]]; then
        echo "[train-marl] ERROR: Mano data not found — run hocap.py first."
