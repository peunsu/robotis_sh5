#!/usr/bin/env bash
# =============================================================================
# evaluate_sequences_shadow.sh — Run rollouts for each HO-Cap sequence with the
# Shadow Hand variant and aggregate metrics.
#
# Mirrors evaluate_sequences.sh exactly, but targets the Shadow-Hand-on-Robotis
# environment (Robotis-Shadow-Grasp-Direct-v0). Checkpoints + metrics live in a
# SEPARATE tree (ffw_shadow) written by train_sequences_shadow.sh, so the
# FFW-SH5 native-hand results remain untouched.
#
# For each sequence, this script:
#   1. Loads the checkpoint from the ffw_shadow checkpoints directory.
...
#   data/processed/hocap/ffw_shadow/right/<trajectory_task>/<data_id>/
#       pretrain.pt / agent.pt          ← checkpoints (written by train_sequences_shadow.sh)
...
# Default: same subset as train_sequences_shadow.sh (subject_1 G10_* + subject_6 G06_*/G09_*).
SEQUENCES=(
...
# Shadow checkpoints live in a separate tree. evaluate.bash automatically
# iterates over all ffw_* subdirectories under data/processed/<dataset>/.
...
for key in "${SEQUENCES[@]}"; do
    ...
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "[eval-shadow ${IDX}/${TOTAL}] ${key}"
    echo "  object=${OBJECT_ID}  traj=${TRAJECTORY_TASK}  id=${DATA_ID}"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

    if [[ ! -f "${CKPT_FILE}" ]]; then
        echo "[eval-shadow] ERROR: Checkpoint not found — run train_sequences_shadow.sh first."
        echo "       ${CKPT_FILE}"
        continue
    fi

    if [[ -f "${OUT_DIR}/metrics.csv" && "${FORCE}" -eq 0 ]]; then
        echo "[eval-shadow] metrics.csv already exists — skipping rollout.  (set FORCE=1 to override)"
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "[eval-shadow] Aggregating metrics for dataset '${DATASET}' ..."
