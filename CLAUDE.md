...
- **EMA action smoothing** (TJ/rl_games convention, `alpha = new action weight`):
  `smoothed = alpha * raw_new + (1 - alpha) * prev_smoothed`. Default `action_smoothing=0.3`
  (≡ legacy alpha-prev value 0.7, same behavior). Lower alpha = smoother / laggier.
...
| `action_smoothing` | `0.3` | EMA weight on the new (raw) action; lower = smoother (TJ convention) |
