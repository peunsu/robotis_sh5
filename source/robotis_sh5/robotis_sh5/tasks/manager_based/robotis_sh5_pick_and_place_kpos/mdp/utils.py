...
def compute_finger_kpos_error(env, command, command_term):
    robot = env.scene[command_term.cfg.asset_name]
    
    num_envs = env.num_envs
    num_fingers = len(command_term.robot_finger_indices)
    
    # 1. 현재 로봇 손가락 위치 추출 (World Frame)
    # (num_envs, num_fingers, 3) -> (num_envs * num_fingers, 3)로 펼치기
    current_kpos_w = robot.data.body_state_w[:, command_term.robot_finger_indices, :3].reshape(-1, 3)
    
    # 2. 로봇 Root 정보를 손가락 개수만큼 복사 (Transform 연산을 위해)
    # (num_envs, 3) -> (num_envs * num_fingers, 3)
    root_pos_w = robot.data.root_pos_w.repeat_interleave(num_fingers, dim=0)
    root_quat_w = robot.data.root_quat_w.repeat_interleave(num_fingers, dim=0)
    
    # 3. Base(Root) 기준 상대 좌표 변환
    # dummy_quat는 위치만 계산하기 위해 임시로 넣어주는 값이야.
    dummy_quat = torch.tensor([1.0, 0.0, 0.0, 0.0], device=env.device).repeat(current_kpos_w.shape[0], 1)
    
    curr_kpos_b_flat, _ = subtract_frame_transforms(
        root_pos_w, root_quat_w, current_kpos_w, dummy_quat
    )
    
    # 4. Target 위치와 비교 (Flatten 상태에서 연산)
    # command에서 [7: 7 + num_fingers * 3] 추출
    target_kpos_b_flat = command[:, 7 : 7 + (num_fingers * 3)]
    
    # 현재 위치를 다시 (num_envs, num_fingers * 3) 형태로 재배열
    curr_kpos_b_reshaped = curr_kpos_b_flat.view(num_envs, -1)
    
    # 5. 오차 계산 (L2 Norm)
    delta_kpos = target_kpos_b_flat - curr_kpos_b_reshaped
    return torch.norm(delta_kpos, p=2, dim=-1)