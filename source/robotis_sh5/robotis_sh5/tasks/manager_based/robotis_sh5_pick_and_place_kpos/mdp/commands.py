...
class DexYCBCommandTerm(CommandTerm):
    def __init__(self, cfg: DexYCBCommandTermCfg, env: ManagerBasedRLEnv):
        ...
        dataset_links = self.traj["link_names"] # 여기서 가져온 이름 리스트 사용
        ...
        for i, name in enumerate(self.robot.body_names):
            if name in dataset_links:
                ...
                dataset_finger_indices.append(dataset_links.index(name))
        ...
        num_finger_links = len(robot_finger_indices)
        self.rel_kpos = torch.zeros(self.num_envs, num_finger_links, 3, device=self.device)
        self.target_kpos_w = torch.zeros(self.num_envs, num_finger_links, 3, device=self.device)
        self.target_kpos_b = torch.zeros(self.num_envs, num_finger_links, 3, device=self.device)
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.rel_kpos[env_ids] = self.traj["kpos"][self.dataset_finger_indices, :] # (num_fingers, 3)
    ...
    def _update_command(self):
        ...
        # [기본] T_world_target = T_world_obj * T_obj_hand
        ...
        num_fingers = self.rel_kpos.shape[1] # 15

        # --- 2. combine_frame_transforms (3D -> 2D 변환 후 수행) ---
        # 물체 포즈 확장
        obj_pos_w_expanded = obj_pos_w.unsqueeze(1).repeat(1, num_fingers, 1).view(-1, 3)
        obj_quat_w_expanded = obj_quat_w.unsqueeze(1).repeat(1, num_fingers, 1).view(-1, 4)
        
        # 상대 좌표(rel_kpos) 평탄화
        rel_kpos_flat = self.rel_kpos.view(-1, 3)
        # 상대 회전은 0이므로 Identity Quaternion으로 채워줌 (W, X, Y, Z)
        rel_quat_flat = torch.zeros((num_envs * num_fingers, 4), device=obj_pos_w.device)
        rel_quat_flat[:, 0] = 1.0 

        # 변환 수행 (N*15, 3)
        goal_kpos_w_flat, _ = combine_frame_transforms(
            obj_pos_w_expanded, 
            obj_quat_w_expanded, 
            rel_kpos_flat, 
            rel_quat_flat
        ...
        # 다시 3D로 복구
        goal_kpos_w = goal_kpos_w_flat.view(num_envs, num_fingers, 3)
        self.target_kpos_w = goal_kpos_w # (num_envs, num_fingers, 3)

        # --- 3. subtract_frame_transforms (3D -> 2D 변환 후 수행) ---
        # 로봇 베이스 포즈 확장 및 평탄화
        root_pos_w_flat = self.robot.data.root_pos_w.unsqueeze(1).repeat(1, num_fingers, 1).view(-1, 3)
        root_quat_w_flat = self.robot.data.root_quat_w.unsqueeze(1).repeat(1, num_fingers, 1).view(-1, 4)
        
        # 목표 손가락 위치(World) 평탄화
        goal_kpos_w_flat = goal_kpos_w.view(-1, 3)
        # 목표 방향은 고려하지 않으므로 Identity로 채움
        target_quat_flat = torch.zeros_like(root_quat_w_flat)
        target_quat_flat[:, 0] = 1.0

        # 변환 수행 (N*15, 3)
        goal_kpos_b_flat, _ = subtract_frame_transforms(
            root_pos_w_flat, 
            root_quat_w_flat,
            goal_kpos_w_flat, 
            target_quat_flat
        ...
        # 최종 결과 저장 (N, 15, 3)
        self.target_kpos_b = goal_kpos_b_flat.view(num_envs, num_fingers, 3)
    ...
    def command(self) -> torch.Tensor:
        # 7(Pose) + 45(Kpos) + 3(Target Pos) = 55차원 명령어 반환
        command = torch.cat([self.pose_command_b, self.target_kpos_b.view(self.num_envs, -1), self.target_pos_w], dim=-1)
        return command
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        if debug_vis:
            ...
            if not hasattr(self, "keypoint_pose_visualizer"):
                self.keypoint_pose_visualizer = VisualizationMarkers(self.cfg.keypoint_pose_visualizer_cfg)
            self.goal_pose_visualizer.set_visibility(True)
            self.current_pose_visualizer.set_visibility(True)
            self.target_pos_visualizer.set_visibility(True)
            self.keypoint_pose_visualizer.set_visibility(True)
        else:
            if hasattr(self, "goal_pose_visualizer"):
                self.goal_pose_visualizer.set_visibility(False)
                self.current_pose_visualizer.set_visibility(False)
                self.target_pos_visualizer.set_visibility(False)
                self.keypoint_pose_visualizer.set_visibility(False)
    ...
    def _debug_vis_callback(self, event):
        ...
        kpos_translations = self.target_kpos_w.view(-1, 3) 
        
        # 2. 쿼터니언 데이터도 위치 데이터 개수와 동일하게 맞춰서 생성
        num_markers = kpos_translations.shape[0]
        kpos_orientations = torch.tensor([1, 0, 0, 0], device=self.device).repeat(num_markers, 1)

        # 3. 시각화 호출
        self.keypoint_pose_visualizer.visualize(kpos_translations, kpos_orientations)
...
class DexYCBCommandTermCfg(CommandTermCfg):
    ...
    keypoint_pose_visualizer_cfg: VisualizationMarkersCfg = FRAME_MARKER_CFG.replace(
        prim_path="/Visuals/Command/keypoint_poses",
        markers={
