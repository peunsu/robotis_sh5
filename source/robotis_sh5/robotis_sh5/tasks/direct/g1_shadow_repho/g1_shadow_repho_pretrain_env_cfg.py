...
Subclasses the train cfg — SAME obs/action dims (765/100) so `pretrain.pt` transfers to the
...
from .g1_shadow_repho_env_cfg import G1ShadowRephoEnvCfg


@configclass
class G1ShadowRephoPretrainEnvCfg(G1ShadowRephoEnvCfg):
    # Pretrain never consumes a warm-start cache (it dumps one for the train phase).
    pretrain_cache_warmstart: bool = False
    # RSI is REQUIRED here regardless of what the train cfg is set to: the whole point of pretrain
    # is to visit every frame of the clip so the dumped state cache covers the full trajectory.
    # Pinned so a temporary train-side `use_rsi=False` A/B does not silently collapse pretrain to
    # frame-0-only starts (which would dump a cache with a single covered frame).
    use_rsi: bool = True
    ...
    term_ft_err: float = 0.10              # m, mean fingertip tracking error (= grasp max_ft_mean_err) set to 0.10 (lower than train 0.15)
    term_wrist_pos_err: float = 0.15       # m, wrist POSITION deviation, worst-of-two-hands (= grasp max_wrist_pos_err)
