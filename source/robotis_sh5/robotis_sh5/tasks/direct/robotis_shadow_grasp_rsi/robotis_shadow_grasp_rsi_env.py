...
class RobotisShadowGraspRsiEnv(DirectRLEnv):
    ...
    def _build_object_cfg(self, cfg: RobotisShadowGraspRsiEnvCfg) -> RigidObjectCfg:
        ...
        return RigidObjectCfg(
            ...
            spawn=sim_utils.UsdFileCfg(
                ...
                # `scripts/process_dataset/assets/convert_obj_to_usd.py`. Isaac Lab's
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        if self.cfg.hand_delta_action:
            hand_delta_cmd = joint_actions[:, :N_f] * self.cfg.hand_delta_scale
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Reference OUTWARD surface normal at the contact vertex (world). Computed once and
        # shared by the force-projection direction (i) AND the front/back contact gate (ii).
        ...
        gn_norm = torch.norm(gn, dim=-1, keepdim=True)                         # (B,5,1)
        gn_unit = gn / gn_norm.clamp(min=1e-6)                                 # (B,5,3) unit outward normal
        gn_valid = gn_norm.squeeze(-1) > 1e-6                                  # (B,5) non-degenerate (mesh present)
        ...
        #      With use_contact_normal_gate, split the vertex→contact displacement into a
        #      TANGENTIAL part (along the surface) and a NORMAL part (front/back, along gn),
        #      and reject contacts pushed BEHIND the surface (opposite face). A single
        #      distance gate cannot catch this on a small object (whole object <
        #      contact_match_dist). Degenerate ref normal → isotropic distance fallback.
        if self.cfg.use_contact_point_gate:
            cp = self._get_fingertip_contact_pos_w()                           # (B,5,3) NaN where no contact
            delta = cp - ref_vertex_world                                      # (B,5,3)
            if self.cfg.use_contact_normal_gate:
                d_n = (delta * gn_unit).sum(dim=-1)                            # (B,5) front(+)/back(-) offset
                d_t = torch.norm(delta - d_n.unsqueeze(-1) * gn_unit, dim=-1)  # (B,5) along-surface offset
                near = (d_t < self.cfg.contact_match_dist) & (d_n > -self.cfg.contact_normal_tol)
                iso = torch.norm(delta, dim=-1) < self.cfg.contact_match_dist  # (B,5) degenerate-normal fallback
                contact_condition = torch.where(gn_valid, near, iso).float()
            else:
                cp_dist = torch.norm(delta, dim=-1)                            # (B,5)
