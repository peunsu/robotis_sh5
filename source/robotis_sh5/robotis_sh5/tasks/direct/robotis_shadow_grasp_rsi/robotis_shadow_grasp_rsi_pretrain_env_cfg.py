...
class RobotisShadowGraspRsiPretrainEnvCfg(DirectRLEnvCfg):
    ...
    # Mirror of the train cfg field — MUST match train so pretrain→train checkpoint
    # transfer keeps the same action dynamics. When True, the ARM action is a
    # per-control-step DELTA (residual), integrated into the target:
    ...
    # Fingers + mass UNAFFECTED. arm_action_smoothing unused while True.
    # Set False to fully restore the original absolute-action arm.
    ...
    # When True, the FINGER action output is a per-control-step DELTA (residual) rather
    # than an absolute target — identical pipeline to the arm delta, on the finger slice.
    # Makes the action semantics CONSISTENT (arm + fingers both velocity/residual) and
    # low-pass-filters finger action noise. MUST match the train cfg for ckpt transfer.
