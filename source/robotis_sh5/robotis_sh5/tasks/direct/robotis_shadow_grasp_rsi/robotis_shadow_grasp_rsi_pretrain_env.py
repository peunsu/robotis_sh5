...
class RobotisShadowGraspRsiPretrainEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: RobotisShadowGraspRsiPretrainEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Adaptive sampling clamps `start_frame` so episodes always run exactly `_num_frame_chunk`
    ...
    def _load_reference_trajectories(self, cfg: RobotisShadowGraspRsiPretrainEnvCfg) -> None:
        ...
        if all(x is not None for x in frame0_arm_list):
            ...
            print(
                ...
                "using default arm pose. Run scripts/process_dataset/retarget/process_arm_pipeline.py first."
            )
    ...
    def _setup_scene(self) -> None:
        ...
        self._contact_sensors: dict = {}  # no object in pretrain; fingertip forces always zero

        # ── Arm ↔ Table contact sensors (anti-cheating; see `rew_arm_contact`) ──
        # Even in pretrain (no physics object), the robot operates near the table and
        # could learn to rest any of `arm_r_link3..link7` on it. We track each link's
        # contact with the table and use the MAX magnitude for soft penalty + termination.
    ...
    def _compute_hand_kpts_pos(self) -> torch.Tensor:
        ...
        Elbow (MANO 21): arm_r_link3 origin + URDF joint4 offset, rotated by link3's
        world quat — stays stable across joint4 rotation (unlike body_pos_w[arm_r_link4]).
        """
        B = self.num_envs
    ...
    def _setup_debug_vis(self) -> None:
        ...
        self._vis_ref_link7 = VisualizationMarkers(
            _sphere_cfg("/Visuals/pretrain/ref_link7", 0.020, (1.0, 1.0, 0.0))
        )
        self._debug_vis_n = n
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        if self.cfg.hand_delta_action:
            hand_delta_cmd = self.actions[:, :N_f] * self.cfg.hand_delta_scale
    ...
    def _get_observations(self) -> dict:
        ...
        ref_kpts_nf = self._ref_mano_kpts[traj, next_frame] + env_orig.unsqueeze(1)  # (B, 23, 3)
        delta_kpts_world = hand_kpts_pos - ref_kpts_nf                               # (B, 23, 3)
        ...
        if self.cfg.debug_vis:
            ...
            ref_link7_pos_vis = self._ref_mano_kpts[traj, frame, 22] + env_orig  # kpt 22 = arm_r_link7
            self._update_debug_vis(ref_ft_next, ft_pos, ref_wrist_pos_vis, ref_elbow_pos_vis, ref_link7_pos_vis)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        ref_kpts_world = self._ref_mano_kpts[traj, frame] + env_orig.unsqueeze(1)  # (B, 23, 3)

        # Keypoint tracking: 21 MANO kpts (Z-weighted) for rew_kpts; arm (elbow+link7) handled separately.
        ...
        early_terminate = ft_err_large | wrist_err_large | wrist_rot_err_large | arm_strong_press
