...
class RobotisSh5GraspMarlEnv(DirectMARLEnv):
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        hand_kpts_pos = self._compute_hand_kpts_pos()                           # (B, 22, 3) world
        ...
        ref_kpts_world_next = self._ref_mano_kpts[traj, next_frame] + env_orig.unsqueeze(1)  # (B, 22, 3)
        ...
        delta_kpts_world = hand_kpts_pos - ref_kpts_world_next                  # (B, 22, 3)
        ...
        # ── Hand obs (289D) — single-agent grasping obs minus mass & lift ────
        hand_obs = torch.cat([
        ...
        # = 63+3+6+3+3+15+27+27+3+6+3+3+63+3+15+3+6+5+27+5 = 289 (21 MANO + elbow separated)

        # ── Shared state (286D) — non-redundant centralized critic input ──────
        ...
        self._shared_state = torch.cat([hand_obs, delta_wrist_rot], dim=-1)  # (B, 292)
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        wrist_err = torch.norm(delta_kpts_mano[:, 0, :], dim=-1)          # unweighted, for termination + rew_wrist_pos
        ...
        # `rew_kpts` averages 21 MANO kpts; wrist gets extra emphasis; elbow is
        # a separate soft-guidance term.
        tracking_penalty = (
